...
def cuda_call(call):
    """
    Wrapper per chiamate CUDA con gestione errori.
    """
...
def memcpy_host_to_device(device_ptr: int, host_arr: np.ndarray):
    """
    Copia dati dalla memoria host alla memoria device.
    """
...
class EngineCalibrator(trt.IInt8EntropyCalibrator2):
    ...
    def set_image_batcher(self, image_batcher: ImageBatcher):
        ...
        self.batch_allocation = cuda_call(cudart.cudaMalloc(size))#common.cuda_call(cudart.cudaMalloc(size))
...
class EngineBuilder:
    ...
    def create_network(self, onnx_path, end2end, conf_thres, iou_thres, max_det, **kwargs):
        ...
        if end2end :
            ...
            # Handle YOLOv8 or similar model outputs
            if v8:
                ...
                bs, num_boxes, num_attributes = previous_output.get_output(0).shape
                print(f"Detected output shape: [batch_size={bs}, num_boxes={num_boxes}, num_attributes={num_attributes}]")

                assert num_attributes >= 5, f"Unexpected number of attributes: {num_attributes}"

                # Slice bounding boxes: [x, y, w, h]
                shapes = trt.Dims([bs, num_boxes, 4])
                boxes = self.network.add_slice(previous_output.get_output(0), starts, shapes, strides)

                # Slice object confidence: [obj_conf]
                starts[2] = 4
                shapes[2] = 1
                obj_conf = self.network.add_slice(previous_output.get_output(0), starts, shapes, strides)

                # Slice class scores if present: [cls_scores]
                if num_attributes > 5:
                    starts[2] = 5
                    shapes[2] = num_attributes - 5
                    cls_scores = self.network.add_slice(previous_output.get_output(0), starts, shapes, strides)
                else:
                    cls_scores = None

                # Debug output for validation
                print(f"Boxes shape: {boxes.get_output(0).shape}")
                print(f"Object confidence shape: {obj_conf.get_output(0).shape}")
                if cls_scores:
                    print(f"Class scores shape: {cls_scores.get_output(0).shape}")
                else:
                    print("No class scores in the output.")
            ...
            layer = self.network.add_plugin_v2([boxes.get_output(0), obj_conf.get_output(0)], nms_layer)

...
def download_video_from_link1(video_url, download_folder="download", log_file="log.txt", cookies_file="cookies.txt"):
...
def download_video_from_link(video_url, download_folder="download", log_file="log.txt"):
    """
    Scarica SOLO video H.264 (avc1), max 1080p, SENZA audio,
    direttamente nella cartella 'download'.
    """
    script_dir = os.path.dirname(os.path.abspath(__file__))
    download_path = os.path.join(script_dir, download_folder)
    ...
    # Leggi log dei video già scaricati
    if os.path.isfile(log_path):
        with open(log_path, "r", encoding="utf-8") as f:
            downloaded_files = f.read().splitlines()
    else:
    ...
    # Otteniamo il titolo sicuro prima di scaricare
    with yt_dlp.YoutubeDL({"quiet": True, "noplaylist": True}) as ydl:
        info = ydl.extract_info(video_url, download=False)
        video_title = info.get("title", "Unknown Video")
        safe_title = sanitize_filename(video_title)

        if safe_title in downloaded_files:
            print(f"⏩ '{video_title}' già scaricato, skipping.")
            return

    # Template output usa il titolo sanitizzato
    ydl_opts = {
        "format": "bestvideo[vcodec^=avc1][height<=1080]/best[vcodec^=avc1][height<=1080]",
        "outtmpl": os.path.join(download_path, f"{safe_title}.%(ext)s"),  # <-- QUI
        "quiet": False,
        "noplaylist": True,
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        print(f"🔄 Download H.264 1080p (NO audio): {video_title}")
        ydl.download([video_url])
...
def extract_frames(
    ...
    frames_folder="download/frames"
):
    base_dir = os.path.dirname(os.path.abspath(__file__))
    download_dir = os.path.join(base_dir, download_folder)
    frames_dir = os.path.join(base_dir, frames_folder)
    ...
    for video in videos:
        input_path = os.path.join(download_dir, video)
        ...
        ffmpeg_cmd = [
            "ffmpeg",
            "-hwaccel", "dxva2",                      # GPU AMD
            "-i", input_path,
            "-an",                                   # niente audio
            "-vf", f"select='not(mod(n\\,{every_n_frames}))'",
            "-vsync", "vfr",
            os.path.join(frames_dir, f"{name}_%06d.jpg")
        ]
...
def annotate_images1(FRAMES_PATH):
...
def annotate_images(FRAMES_PATH):
    """
    Annotazione delle immagini con YOLO-ready txt.
    Ridimensiona le immagini all'input del modello ONNX prima della predizione.
    
    Args:
        FRAMES_PATH (str): cartella contenente le immagini.
        model: modello ONNX già caricato (wrapper che ritorna risultati).
        onnx_path (str): path al file .onnx per ottenere lo shape.
    """
    # Ottieni dimensione input del modello
    #h, w = get_onnx_input_size(onnx_path)
    ...
    for filename in tqdm(os.listdir(FRAMES_PATH), desc="Annotating images"):
        ...
        try:
            ...
            img_width, img_height = image_orig.size  # dimensione originale
        except Exception as e:
            print(f"❌ Errore nell'aprire l'immagine {filename}: {e}")
            continue  # Salta le immagini corrotte

        # Ridimensiona immagine per il modello
        image_resized = image_orig.resize((w, h))
        image_array = np.asarray(image_resized)

        # Predizione
        results = model(image_array)  # ritorna box, score, class_id, cls
        annotation_lines = []

        for box, score, class_id, cls in results:
            x1, y1, x2, y2 = box  # coordinate sul frame ridimensionato

            # Ri-calcola in base alle dimensioni ORIGINALI
            x_scale = img_width / w
            ...
            width = (x2_orig - x1_orig) / img_width
            height = (y2_orig - y1_orig) / img_height
...
modelname='2classi.onnx'
min_conf=0.3
max_iou=0.5
model = ONNX(modelname, min_conf, max_iou)
# Ottieni dimensione input del modello
h, w = get_onnx_input_size(modelname)
#extract_frames(every_n_frames=10)

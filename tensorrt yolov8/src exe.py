...
class Utility:
    ...
    def det_postprocess6(self, data: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor], confidence_threshold: float = 0.50, class_id: int = None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:

        num_dets, bboxes, scores, labels = (i[0] for i in data)
        # Applica i filtri di soglia di confidenza e ID della classe
        selected = (scores >= confidence_threshold)
        if class_id is not None:
            selected = selected & (labels == class_id)
...
while True:   
    ...
    if fpscount:
        utility.count_fps(start_time, fps_counter)

...
import pickle
import warnings
from typing import List, OrderedDict, Tuple, Union, Optional
import torch
import numpy as np
...
class Utility:
    ...
    def seg_postprocess(self, data, shape, conf_thres=0.25):
        assert len(data) == 2  # Ensure data contains two elements
        outputs, proto = data[0][0], data[1][0]
        h, w = shape[0], shape[1]  # Use provided shape directly for upsampling

        # Split the output into its respective components
        bboxes, scores, labels, maskconf = outputs.split([4, 1, 1, 32], dim=1)
        ...
        if not selected.any():
            return torch.empty(0, 4), torch.empty(0), torch.empty(0), torch.empty(0, h, w, 1)

        # Select data based on the threshold
        bboxes_selected = bboxes[selected]
        scores_selected = scores[selected]
        labels_selected = labels[selected]
        maskconf_selected = maskconf[selected]

        # Generate masks using the selected prototype masks
        masks = (maskconf_selected @ proto).sigmoid().view(-1, h, w)
        masks = F.interpolate(masks.unsqueeze(0), shape, mode='bilinear', align_corners=False).squeeze(0)
        masks = masks.gt(0.5).unsqueeze(-1)

        return bboxes_selected, scores_selected, labels_selected, masks
